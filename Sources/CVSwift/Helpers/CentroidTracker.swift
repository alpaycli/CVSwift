...
class CentroidTracker {
    ...
    var maxDisappearedFrameCount: Int = CentroidTracker.maxDisappearedFrameCount
    // max normalized distance(in an image coordinate space) for an rectangle to be considered a different object
    var maxNormalizedDistance: CGFloat = CentroidTracker.maxNormalizedDistance
    
    private var nextObjectId: Int = 0

    // currently tracked object
    var objects: [Int: CGRect] = [:]
    ...
    // objects in frame
    var objectsInFrame: [Int: CGRect] {
        return objects.filter({!tempDisappearedObjects.filter({$0.value > 0}).keys.contains($0.key)})
    }
    
    // object deregistered id list
    var deregisteredObjects: [Int] = []
    ...
    func registerObject(at rect: CGRect) {
...
//        print("deRegisterObject: \(objectID)")
        ...
        self.deregisteredObjects.append(objectID)
    }
...
//        print("currently tracking")
        ...
        let objectCentroids: [CGPoint] = Array(self.objects.values).map({CGPoint(x: $0.midX, y: $0.midY)})
        ...
        for i in 0..<rowCount {
            ...
            for j in 0..<colCount {
                let pointB = CGPoint(x: rects[j].midX, y: rects[j].midY)
